function ballPhysics(step){
    ...
    if(ball.position.x - ball.geometry.boundingSphere.radius<= -fieldWidth/2){
        ...
        if(!invincible){
            ...
            if(playerH == 0 && level<maxLevel){
                    ...
                    speed = baseSpeed;
                    reset_bonuses();
                    document.getElementById("info").textContent="You lost :( press space to try again";
    ...
    let ray2 = new THREE.Raycaster(ball.position, new THREE.Vector3(speedX,speedY, 0).applyAxisAngle(new THREE.Vector3(0,0,1),THREE.Math.degToRad( 33 )).normalize());
    let ray3= new THREE.Raycaster(ball.position, new THREE.Vector3(speedX,speedY, 0).applyAxisAngle(new THREE.Vector3(0,0,1), THREE.Math.degToRad( -33 )).normalize());
